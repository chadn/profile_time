exports = module.exports = (function (){
	 ...
	 */
	var Profiler = function (url, opts) {
		if ( !(this instanceof Profiler) ) {
			return new Profiler( url, opts );
		}
	...
	Profiler.prototype.addLineToTotal = function(line, total) {
		...
		tmp = line.json.Done || line.json['e:ALL']; // time for this requiest
		if (this.options.skipTimeouts && this.options.skipTimeouts < tmp) {
			return;
		}
		...
		if (this.options.urls) {
			...
			for (key in line.elapsedMs) {
				if (this.options.skipTimeouts && this.options.skipTimeouts < line.elapsedMs[key]) {
					continue;
				}
				add2Key(total.totalLines, key, 1);
	...
	Profiler.prototype.summarizeTotal = function() {
		...
		if (this.options.top) {
			...
			topKeys = this.sortObjByValue(this.total[this.options.top]);

			if (this.options.topN > 0) {
				topKeys.reverse();
				// add 1 since 'ALL' topKey will always be there.
				topKeys.splice(1 + this.options.topN, topKeys.length);
			} else {
				topKeys.splice(1 - this.options.topN, topKeys.length);
			}

			// make sure 'ALL' made it, needed for computeAvgPercent
			if (!in_array('ALL', topKeys)) {
				// replace least 'top' one with 'ALL'
				topKeys.splice(-1, 1, 'ALL');
			}
			//console.log("topKeys:", this.options.topN, topKeys.length, topKeys);
